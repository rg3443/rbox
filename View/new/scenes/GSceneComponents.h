...
#include <SDL_image.h>
#include "../../../pcgGlobal.h"
#include "../../../model/CDialogue/CDialogue.h"
//special types
...
struct SFont {
    ...
    SFont(string filePath_,int size__) {
        this->Open(filePath_,size__);
    }

    //methods
    bool isOpen() { return iop; }
    void Open(string filePath_,int size__) {
...
struct SText {
    ...
    //
    SText() {}
    SText(string text_,int fsize,SRGB color_) { //part constructor
        text = text_;
        color = color_;
        fontSize = fsize;
    ...
    SText(string text_,int fsize, SRGB color_,SSquare sqr) { //independent constructor
        text = text_;
        fontSize = fsize;
        color = color_;
        defaultFontSize = false;
...
//                //printf("table tile data\n");
    ...
    void SetButtonText(SText text,int row, int column) {
        ...
        //todo: change button size(and stext) н.о. text size
        buttonMatrix[row][column].text = text;

        cout << "текст =" <<  (text.text.size()) << endl;

        buttonMatrix[row][column].text.textField = buttonMatrix[row][column].sqrFld;

    }
    void SetColumnName(int id, string text) {
    ...
    int ChangeColumnW(int val,int colid) {
        val = to_negative(val);
        int newW = buttonW;
        printf("старый w %d\n",buttonW);
        for(int column=colid;column<buttonMatrix[0].size();column++) {
            for(int row=0;row<buttonMatrix.size();row++) {
                SPoint currentdrPoint = buttonMatrix[row][column].sqrFld.drPoint;
                SPoint currentulPoint = buttonMatrix[row][column].sqrFld.ulPoint;
                if(column == colid) {
                    //on;y drPoint
                    buttonMatrix[row][column].sqrFld.drPoint.Move( currentdrPoint.x+val,currentdrPoint.y );
                    printf("двигаем точку по w на %d пикселей, нвоое значение = %d\n",val,(currentdrPoint.x+val));
                }
            }
        }
        return newW+val;
    }
    void SetColumnW(int newW,int columnid) {
        for(int row=0;row<buttonMatrix.size();row++) {
            for(int column=0;column<buttonMatrix[row].size();column++) {
                if(column == columnid) {
                    SPoint buttonULP = buttonMatrix[row][column].sqrFld.ulPoint;
                    buttonMatrix[row][column].sqrFld.drPoint.x = buttonULP.x+newW;
                }
            }
        }
    }
    void SetButtonDirection(int type,int val,int row,int column) {
        buttonMatrix[row][column].type = type;
        buttonMatrix[row][column].directionCode = val;
    }
    void PrintData() {
//        printf("\tTABLE DATA\n");
...
struct STexture {
    ...
    STexture() {}
    STexture(int code,string imgFilePath_,SSquare imgField_) {
        this->Init(code,imgFilePath_,imgField_);
    }
    void Init(int code,string imgFilePath_,SSquare imgField_) {
...
struct SDialogBlock {
    CDialogue* object;
    SText question;
    vector<SButton> answerList;

    SDialogBlock() {}
    SDialogBlock(CDialogue* object_) {
        object = object_;
      //object->Get
    }
    int GetDialogId() { return object->GetId(); }
    bool IsDone() { return object->IsDone(); }
};
